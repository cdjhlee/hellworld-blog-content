...
## 소개
Yarn은 npm 쓰다가 ~~빡친~~불만을 가진 사람들이 만든 node.js 패키지 매니저입니다. 하필 YARN이 하둡에서도 쓰이는지라 구글 검색하면 죄다 하다둡 글만 나옵니다. 저도 모르고 있다가 소개글을 다른 분께 듣고 알게 되었습니다. 그리고 한 번 시범삼아 사용해보고 글을 작성해보고자 합니다.
...
```
`npm install --save` = `yarn add` 이렇게 생각하시면 될 것 같습니다. 하나만 되는 게 아니라 저 뒤로 줄줄이 소세지로 묶어서 설치할 수 있습니다. 한 번 hapi를 추가해보겠습니다. 요즘 개인프로젝트에 hapi를 쓸까 고려중이라서요. `yarn add hapi` 로 설치해봅시다.
